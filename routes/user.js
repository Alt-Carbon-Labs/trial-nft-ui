...
/* Implement UpgradePremium Endpoint */
router.post("/upgrade-premium", async (req, res) => {
  console.log("reached all the way here");
  if (req.isAuthenticated()) {
    await users.update(
      { issuer: req.user.issuer },
      { $set: { isPremium: !req.user.isPremium } }
    );
    return res.status(200).end(`User is now premium.`);
  } else {
    return res.status(401).end(`User is not logged in.`);
  }
});
...
/* Implement Buy Apple Endpoint */
router.post("/sell-apples", async (req, res) => {
  if (req.isAuthenticated()) {
    await users.update(
      { issuer: req.user.issuer },
      { $set: { appleCount: 0 } }
